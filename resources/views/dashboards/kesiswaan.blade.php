...
<x-layouts.app :title="$title">
    ...
    <section class="mt-6 overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-xs">
            <div class="flex flex-col gap-4 border-b border-slate-100 px-5 py-4 min-[761px]:flex-row min-[761px]:items-end min-[761px]:justify-between">
                ...
                <form method="GET" action="{{ route('kesiswaan.dashboard') }}" class="grid grid-cols-2 gap-2 min-[461px]:grid-cols-[140px_140px_auto_auto] min-[461px]:items-end">
                    ...
                    @if(request()->hasAny(['dari', 'sampai']))<a href="{{ route('kesiswaan.dashboard') }}" class="inline-flex h-9 items-center justify-center rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-600">Reset</a>@endif
                </form>
            </div>
